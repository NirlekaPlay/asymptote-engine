...
weapons, dead bodies, hostages, and damaged property.
 ...
 a. Detection Time. Is the amount of time a suspicion value
   ...
   `FinalDetectionTime = BaseTime ÷ SpeedModifier %`
 b. Detection Priority. As defined in Sec. 1, the speed of
    detection and how an Agen reacts upon detection is de-
    termined by the Player's highest priority status.
 c. Detection Visibility. For circumstances such as an un-
    disguised Player walking in a trespassing zone, and
    being heard by an Agent, that Agent cannot see the
    Player's current statuses. Therefor Some statuses
    must require line of sight.
