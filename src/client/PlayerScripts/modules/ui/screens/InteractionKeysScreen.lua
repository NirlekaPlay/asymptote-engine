...
local keyPairs: { [string]: { key: Frame, action: Frame } } = {}

--[=[
	@class InteractionKeysScreen
]=]
local InteractionKeysScreen = {}

function InteractionKeysScreen.showInteractionPair(id: string, keyStr: string, actionStr: string): ()
	InteractionKeysScreen.removeInteractionPair(id)

	keyPairs[id] = InteractionKeysScreen.createNewPair(keyStr, actionStr)
end

function InteractionKeysScreen.removeInteractionPair(id: string): ()
	local existing = keyPairs[id]
	if existing then
		existing.action:Destroy()
		existing.key:Destroy()
		keyPairs[id] = nil
	end
end

function InteractionKeysScreen.createNewPair(keyStr: string, actionStr: string): { key: Frame, action: Frame }
	...
	return { key = keyTextFrame, action = actionTextFrame }
end
